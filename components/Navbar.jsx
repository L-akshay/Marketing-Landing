...
import { WIGGLE_CONFIG, NAV_WORK_ITEMS } from '@/lib/data';
...
export default function Navbar() {
    useEffect(() => {
        ...
        // Start white (on-dark) — the hero card is a dark surface.
        ...
        const cleanups = [];
        const logo = document.querySelector('.logo-aarohiq');
        if (logo) cleanups.push(initWiggle(logo, WIGGLE_CONFIG.logoAarohiq));
        ...
        if (navLeft && workBox && workBlob) {
            ...
            // Measure both the box AND the blob icon centre so the box grows
            // out of the icon rather than out of its own corner.
        ...
        if (navRight && waBox) {
            ...
            gsap.set(waBox, {
                ...
                scale: 0,
                opacity: 0,
                transformOrigin: `${waOriginX}px ${waOriginY}px`
            });
            gsap.set(waItems, { y: 10, opacity: 0 });

            const onEnterRight = () => {
                gsap.killTweensOf([waBox, waItems]);
                showOverlay();
                if (waSvgPath) gsap.to(waSvgPath, { fill: '#0e6634', duration: 0.3 });
        ...
        // ─── Work item: badge wiggle + tile tilt on hover ───
        document.querySelectorAll('.nav-work-item').forEach(item => {
            const badge = item.querySelector('.nav-work-badge');
            const tile = item.querySelector('.nav-work-item__img');
            let wiggleTween;
            ...
            const onItemEnter = () => {
                ...
                if (tile) gsap.to(tile, { rotation: 16, scale: 1.15, duration: 0.25, ease: 'power2.out' });
            };
            const onItemLeave = () => {
                if (wiggleTween) wiggleTween.kill();
                if (badge) gsap.to(badge, { rotation: 0, duration: 0.3, ease: 'power2.out' });
                if (tile) gsap.to(tile, { rotation: 0, scale: 1, duration: 0.3, ease: 'power2.out' });
            };
        ...
        // ─── Work button: wiggle intensity 4 ───
            ...
            <nav className="navbar">
                <div className="nav-left" style={{ cursor: "url('/assets/Cursor SVG/cursor-pointer.svg') 12 12, pointer" }}>
                    <div className="nav-hover-trigger">
                        {/* An anchor, not a div: the pop-out only opens on
                            hover, so on touch this has to go somewhere by
                            itself or the navbar is dead on mobile. */}
                        <a href="#showreel-section" className="logo-work-container">
                            <img
                        ...
                        {/* Pop-out: delivered work */}
                        <div className="nav-popout nav-work-box">
                            <div className="nav-popout-inner">
                                {NAV_WORK_ITEMS.map((item) => (
                                    <a href={item.href} className="nav-work-item" key={item.title}>
                                        <div className="nav-work-item__img-wrap">
                                            <span className={`nav-work-item__img nav-work-item__tile nav-work-item__tile--${item.badgeColor.replace('badge-', '')}`} />
                                        </div>
                                        <div className="nav-work-item__text">
                                            <span className={`nav-work-badge ${item.badgeColor}`}>{item.badge}</span>
                                            <h4 className="nav-work-title">{item.title}</h4>
                                        </div>
                                    </a>
                                ))}
                                <a href="#showreel-section" className="nav-work-btn">
                ...
                <div className="nav-center" style={{ cursor: "url('/assets/Cursor SVG/cursor-pointer.svg') 12 12, pointer" }}>
                    <a href="/" className="logo-aarohiq" aria-label="Aarohiq — home" data-wiggle-target="true">
                        aarohiq
                    </a>
                </div>
                ...
                <div className="nav-right" style={{ cursor: "url('/assets/Cursor SVG/cursor-pointer.svg') 12 12, pointer" }}>
                    <div className="nav-hover-trigger">
                        <a
                            href={whatsappUrl || `mailto:${site.email}`}
                            className="logo-whatsapp"
                            aria-label={whatsappUrl ? 'Chat with us on WhatsApp' : `Email ${site.email}`}
                        ...
                        <div className="nav-popout nav-wa-box">
                            <div className="nav-popout-inner">
                                ...
                                <p className="nav-wa-desc">Scan the QR code to chat with us from your phone.</p>
                                <a
                                    href={whatsappUrl || `mailto:${site.email}`}
                                    className="nav-wa-link"
                                    {...(whatsappUrl ? { target: '_blank', rel: 'noopener noreferrer' } : {})}
                                >
                                    <span className="nav-wa-link-text">
                                        {whatsappUrl ? 'Chat via desktop' : site.email}
                                    </span>
